    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                online_tlwhs, online_ids, action_results,color_results  = track_main(tracker, np.array(Results), Result_kpts, frame_id,
                                                                      1080, 1920,
                ...
                for det_index, (output, conf) in enumerate(zip(outputs, det[:, 4])):
                    ...
                    if view_img:  # Add bbox to image
                        label = f'{int(id)} {conf:.2f} {str(action_results[det_index].split(":")[0].strip())}'
    ...
    for target in online_targets:
        ...
        if len(target.keypoints_list) == 30:
            ...
            if action_name == 'Fall Down':
                ...
                clr = (255,0,0)
            color_results.append(clr)
            print(action)
            action = action
        action_results.append(action)

    return online_tlwhs, online_ids, action_results,color_results  # ct+++action_results
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='sittinglaying.mp4', help='source')  # file/folder, 0 for webcam
