...
def plot_one_box(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None):
    ...
    if label:
        ...
        # 计算警告消息 "Warning!!!" 的位置
        warning_text_size = cv2.getTextSize("Warning!!!", 0, fontScale=tl / 6, thickness=tf // 2)[0]
        warning_position = (c1[0] + (t_size[0] - warning_text_size[0]) / 2, c1[1] - t_size[1] - 3)

        cv2.putText(im, label, (c1[0], c1[1] - 2), 0, tl / 6, [225, 255, 255], thickness=tf//2, lineType=cv2.LINE_AA)
        parts = label.split()  # 通过空格分割字符串
        last_part = parts[-1]  # 获取最后一个部分
        if last_part in ['Lying Down', 'Sit down', 'Fall Down','Sitting']:
            cv2.putText(im, "Warning!!!", (int(warning_position[0]), int(warning_position[1])), 0, tl/4, [0, 0, 255], thickness=tf,
